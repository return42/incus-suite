...
# Folder on HOST system to share to GUEST system
LXC_HOST_SHARE="$(cd "${LXC_HOST_SHARE:-${HOME}}" && pwd -P)"
...
copy a LXC image from a remote image server to the local server
  ...
  usage: ${MAIN} ${MAIN_CMD} "<remote-name:image-name>" <local-name>
EOF
}
...
create LXC instance from a local image
  ...
  usage: ${MAIN} ${MAIN_CMD} <image-name> <instance-name>
EOF
}
...
delete LXC instance
  ...
  usage: ${MAIN} ${MAIN_CMD} <instance-name>
EOF
}
...
lxc.instance.del() {
    ...
    if lxc.instance.exists "${instance_name}" &>/dev/null; then
        msg.info "stop & delete instance ${_BBlue}${1}${instance_name}${_creset}"
        incus stop "${instance_name}" &>/dev/null
...
start LXC instance
  ...
  usage: ${MAIN} ${MAIN_CMD} <instance-name> [<sec>]
...
check internet connectivity of the LXC instance
  ...
  usage: ${MAIN} ${MAIN_CMD} <instance-name> [IP]
...
lxc.instance.get() {
    ...
    if [ -z "${out}" ]; then
	msg.err "instance ${instance_name} is unknown"
	return 42
    fi
...
lxc.instance.configure() {
    ...
    lxc.instance.share "${instance_name}" "${LXC_HOST_SHARE}"
}

lxc.instance.share.help() {
    $FMT <<EOF
mount folder from HOST system within instance

  usage: ${MAIN} ${MAIN_CMD} <instance-name> <host-folder> [<basename>]
