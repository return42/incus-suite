...
LXC_ROOT_FOLDER="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)"

# Location in the container where all folders from HOST are mounted
LXC_GUEST_MOUNT="/host-share"

# Folder of this lxc-suite repo on GUEST system

LXC_ROOT_FOLDER_GUEST="${LXC_GUEST_MOUNT}/$(basename "${LXC_ROOT_FOLDER}")"
...
lxc.instance.share() {
	...
	msg.info "device ${guest_folder}_share already exists"
    else
